...
public class ClienteControlador {
    ...
    @PostMapping ("/crear-ofertas")
    public ResponseEntity<MensajeDTO<String>> crearOfertas(@Valid @RequestBody CrearOfertaDTO crearOfertaDTO ) throws Exception {
        negocioServicio.crearOferta(crearOfertaDTO);
        return ResponseEntity.ok().body(new MensajeDTO<>(false, "La oferta se ha creado correctamente"));
    }
    @PostMapping ("/crear-chat")
    public ResponseEntity<MensajeDTO<String>> crearChat(@Valid @RequestBody CrearChatDTO crearChatDTO  ) throws Exception {
        chatServicio.crearChat(crearChatDTO);
        return ResponseEntity.ok().body(new MensajeDTO<>(false, "El chat se ha creado correctamente"));
    }
    @PutMapping ("/enviar-mensaje-chat")
